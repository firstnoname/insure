...
$sql_sub="SELECT insurance_repayment.repay_id, 
...
FROM insurance_relationship_view INNER JOIN insurance_repayment ON insurance_relationship_view.cusno = insurance_repayment.cus_id
	 INNER JOIN main_cus_ginfo ON main_cus_ginfo.CusNo = insurance_repayment.cus_id
	 INNER JOIN insurance_emp_view ON insurance_emp_view.id_card = insurance_repayment.repayment_emp_id
WHERE repayment_cancel=0 and insurance_repayment.repay_id=$id"; 
  ...
  <tr>
    ...
    <td colspan="2" rowspan="16" valign="top"><table width="100%" border="0" cellspacing="1" cellpadding="1">
      <tr>
        ...
        <td width="27%">&nbsp;</td>
        <td width="17%">&nbsp;</td>
        <td width="21%">วันที่<?= date('d',strtotime($rw['repayment_date'])) ?> เดือน <?= $monthNames[date('m',strtotime($rw['repayment_date']))] ?> พ.ศ. <?= date('Y',strtotime($rw['repayment_date']))+543 ?></td>
      </tr>
      <tr>
        <td height="28"></td>
        <td colspan="3">นาม  <strong><?php echo  $rw['Customer'];?></strong></td>
        <td>&nbsp;</td>
      </tr>
      <tr>
        <td height="27">&nbsp;</td>
        <td colspan="3">ที่อยู่ <strong><?=$rw['address_full_name'] ?></strong></td>
        <td> </td>
      </tr>
      <tr>
        <td height="27">&nbsp;</td>
        ...
        <td colspan="4"><table width="100%" border="1" cellspacing="0" cellpadding="0">
          <tbody>
            ...
            <tr>
              ...
              <td>เงินฝาก-ต่อประกันภัย (ค่างวด)</td>
              <td align="center">1</td>
              <td align="right"><?=$rw['repayment_amount'] ?></td>
              <td align="right"><?=$rw['repayment_amount'] ?></td>
            </tr>
            <tr>
            ...
            <tr>
              <td colspan="2" rowspan="2" align="center">(<?=num2wordsThai($rw['repayment_amount']) ?>)</td>
      ...
      <tr>
        ...
        <td align="center"><?=$rw['employee'] ?></td>
