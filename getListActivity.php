...
CASE WHEN ISNULL(insurance_customer_activity.customer_activity_id) THEN 'รอโทร'  ELSE insurance_customer_activity.track_last_status END AS track_last_status, 
CASE WHEN ISNULL(insurance_customer_activity.customer_activity_id) THEN 'info' WHEN insurance_customer_activity.activity_IsClose=1  THEN 'danger' WHEN track_last_status='ไม่สะดวกคุย'  THEN 'warning' WHEN track_last_status='ติดต่อไม่ได้'  THEN 'warning'  ELSE 'success' END AS info, 
insurance_customer_activity.date_appointment,
insurance_customer_activity.activity_Date,
insurance_customer_activity.main_status,
insurance_customer_activity.call_count,
insurance_relationship_view.vehicle_id,
COALESCE(insurance_customer_activity.activity_id,0) As activity_id,
...
COALESCE(insurance_customer_activity.activity_IsClose,0) as activity_IsClose,
...
LEFT JOIN insurance_emp_view ON insurance_customer_activity.emp_id_card = insurance_emp_view.id_card Where   COALESCE(insurance_customer_activity.activity_IsClose,0)=0  ";

if($Activityid<100){
   $query .="  AND COALESCE(insurance_customer_activity.activity_id,0) <>101 ";
}
if($Activityid ==101){
 $query .=" and  track_last_status='สนใจ'  ";
}
if($Activityid ==100){
    $query .=" and   insurance_customer_activity.insurance_Status=1"; 
} 
